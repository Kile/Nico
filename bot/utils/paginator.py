...
class Paginator:
    ...
    async def _start(self) -> View:
        ...
        (await self.interaction.response.send_message(file=self.file, embed=self.embed, view=self.view, ephemeral=self.ephemeral)) if self.file else (await self.interaction.response.send_message(embed=self.embed, view=self.view))
        self.view.message = await self.interaction.original_response()
